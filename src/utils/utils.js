...
// 从hashurl里获取参数
export const getHashString = (name)=>{
  let hashString= window.location.hash;
  let hashText = hashString.substring(hashString.indexOf("?"+name+"=")).replace("?"+name+"=","")
  hashText = decodeURIComponent(hashText);
  return (hashText&&hashText!=undefined&&hashText!='null')?hashText:''
}

//处理处理本地图片路径,以/开头加上.,解决集成到springboot上找不到图片路径问题
export const dealLocalImagePath=(path='',prefix='.')=>{
  if(path && prefix!='./' && !path.includes(prefix) && !path.startsWith('http')){
    path = prefix+path;
  }
  return path
}
